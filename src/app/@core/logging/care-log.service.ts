...
export class CareLogService {
  ...
  setUp(): void {
    this.careAuthenticationService.onUserProfileState$.pipe(first()).subscribe(() => {
  ...
  private sendDebugMessage(message: string, optionalParams: any[]): void {
    if (this.logLevel >= LogLevel.DEBUG) {
      ...
      const content = this.appendParametersAsStringIfPresent(message, optionalParams);
  ...
  private appendParametersAsStringIfPresent(content: string, optionalParams: any[]): string {
    if (optionalParams.length) {
      return `${content}. ${optionalParams.map((param) => JSON.stringify(param)).join(', ')}`;
    }
    return content;
  }
}
