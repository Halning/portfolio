...
@Component({
  ...
  animations: [
    ...
    trigger('homeHello', [
      ...
      transition(':enter', [animate('0.8s 0.5s ease', style({ opacity: 1 }))]),
      transition('inactive => active', [animate('0.8s ease')])
    ]),
    trigger('homeMainTitle', [
      ...
      transition('inactive => active', [animate('0.8s 0.5s ease')])
    ]),
    trigger('homeTitle', [
      state('inactive', style({ opacity: 0, transform: 'translate(0, 20px)' })),
      state('active', style({ opacity: 1, transform: 'translate(0px, 0px)' })),
      ...
      transition('inactive => active', [animate('0.8s 1s ease')])
    ])
  ]
})
