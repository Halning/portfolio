...
export class BlockLoaderComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.defaultParentPosition = this.windowRefSrv.nativeWindow.getComputedStyle(this.elRef.nativeElement.parentElement).position;
        this.elRef.nativeElement.parentElement.style.position = (this.defaultParentPosition === 'absolute') ? 'absolute' : 'relative';
        this.checkIfParentHasBorders();
    }

    private checkIfParentHasBorders(): void {
        const borderWidth = parseFloat(this.windowRefSrv.nativeWindow.getComputedStyle(this.elRef.nativeElement.parentElement).borderWidth);
        if (!borderWidth) {
            return;
        }
